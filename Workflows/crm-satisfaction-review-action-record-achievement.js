...
// on transition: InProgress -> InProgress (Record an achievement)
...
// create new Achievement ticket
def now = Calendar.instance
def achievementDate = "${now.get(Calendar.YEAR)}.${String.format('%02d', 1 + now.get(Calendar.MONTH))}"
...
def result = post("/rest/api/3/issue")
    ...
    .body([
        fields:[
            ...
            summary: "Achievements of ${projectName} on ${achievementDate}",
...
if(result.status < 200 || result.status >= 300) {
    logger.info("Could not create achievement for satisfaction review ${issue.key} (${result.status})")
    return
}

def newTicket = result.body as Map
logger.info("Created achievement ${newTicket.key} for project ${projectName}")

// clear the achievement details from the satisfaction review ticket, so new achievements can be created clean
...
if(result.status < 200 || result.status >= 300)
    logger.info("Could not clear achievement details from satisfaction review ${issue.key} (${result.status})")

// add a comment about the new achievement that was created

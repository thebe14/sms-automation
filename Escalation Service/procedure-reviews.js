// schedule: Every day
// jql: issuetype=Procedure AND status=Active AND "Review frequency[Dropdown]" not in (null, "Together with process review") AND "Next review[Date]" < now()
// run as: LF
...
if(result.status < 200 || result.status > 204) {
    logger.info("Could not get procedure ${issue.key} (${result.status})")
    return
}

def procedure = result.body
...
// transition procedure to status In Review
...
if(result.status < 200 || result.status > 204) {
    logger.info("Could not transition procedure ${issue.key} via ${transName} (${result.status})")
    return
}

logger.info("Started review of procedure ${issue.key}")
